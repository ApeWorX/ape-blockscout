...
from ape_blockscout.exceptions import BlockscoutResponseError, get_request_error
...
class BlockscoutResponse:
    ...
    def value(self) -> ResponseValue:
        try:
            ...
            # Blockscout may respond with HTML content.
            raise BlockscoutResponseError(self.response, "Resource not found") from err
        ...
        if result.startswith("Error!"):
            ...
            if self.raise_on_exceptions:
                raise BlockscoutResponseError(self.response, err_msg)
